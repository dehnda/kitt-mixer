...
type Props = { onBack: () => void; cocktails: Cocktail[] };

export default function ShoppingGuide({ onBack, cocktails }: Props) {
    ...
    const makeable = useMemo(() => {
        if (selected.size === 0) return [] as Cocktail[];
    ...
    const usedInMap = useMemo(() => {
        const map = new Map<string, number>();
        allIngredients.forEach((ing) => {
            const count = cocktails.filter((c) =>
