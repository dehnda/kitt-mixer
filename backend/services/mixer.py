...
class MixerService:
    ...
    def get_available_cocktails(self) -> List[CocktailWithAvailability]:
        """Get all cocktails with availability based on installed liquids"""
        cocktails = self.db.get_cocktails()
        installed_liquids = set(self.db.get_installed_liquids())
        ...
        for cocktail_data in cocktails:
            ...
            # Check availability
            required_liquids = {ing.ingredient for ing in ingredients}
            missing = required_liquids - installed_liquids
    ...
    def can_make_cocktail(self, cocktail_name: str) -> tuple[bool, List[str]]:
        """
        Check if a cocktail can be made
        ...
        installed_liquids = set(self.db.get_installed_liquids())
        required_liquids = {ing['ingredient'] for ing in cocktail_data.get('ingredients', [])}
        missing = required_liquids - installed_liquids
